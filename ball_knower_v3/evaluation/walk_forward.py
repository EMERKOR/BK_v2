...
def split_frame(df: pd.DataFrame, fold: WalkForwardFold, *, time_col: str):
    ...
    times = pd.to_datetime(df[time_col], utc=True, errors="raise")
    train = df.loc[times <= fold.train_end].copy()
    test = df.loc[(times >= fold.test_start) & (times <= fold.test_end)].copy()
...
def pinball_loss(y_true, y_pred_quantile, *, quantile: float) -> float:
    ...
    if y.shape != q.shape or y.size == 0:
        raise ValueError("quantile predictions must match a non-empty target array")
