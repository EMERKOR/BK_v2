...
PIPELINE_SCHEMA = "phase3c_prospective_bundle_v1"
REGISTRY_SCHEMA = "phase3c_prospective_registry_v1"
...
def _manifest(root: Path, *, spec: dict, source_receipts: dict, code_commit: str) -> dict:
    ...
    content = {
        ...
        "origin": _utc(spec["origin"], "origin").isoformat(),
        ...
        "attestation_status": "not_attested_local_build",
...
def verify_bundle(
    ...
    *,
    ...
    origin = _utc(content["origin"], "manifest origin")
    ...
    if require_attestation:
        ...
        archive = Path(attestation.get("subject_path", ""))
        ...
        attested_at = _utc(attestation.get("attested_at"), "attested_at")
        if (pd.to_datetime(target_structural.kickoff, utc=True) <= attested_at).any():
            raise ValueError("GitHub attestation was not completed before every target kickoff")
        ...
        command = [gh_executable, "attestation", "verify", str(archive), "--repo", repository]
...
def append_registry(
    ...
    *,
    ...
    """Append one attested origin; corrections require a valid supersedes link."""
    ...
    for entry in existing:
        ...
        if entry.get("record_type") == "registry_genesis":
            continue
        known_digests.add(entry["bundle_digest"])
        if entry["origin"] == content["origin"]:
            same_origin.append(entry)
    if verification["bundle_digest"] in known_digests:
        raise ValueError("bundle is already registered")
...
def main() -> None:
    ...
    if args.command == "build":
        ...
        result = append_registry(
            args.registry, args.bundle, args.attestation_receipt, repository=args.repository
        )
