...
# 12. Reviewed stopping point and new-thread handoff

This is the planned stopping point for the current design thread.

The architecture should now move from serial research resolution to **independent implementation/readiness review and minimum benchmark implementation**.

Priority next-thread questions:

1. Specify/implement the reviewed team-state benchmark ladder.
2. Build the direct game-distribution baseline without premature weather/key-number/QB complexity.
3. Design the QB representation promotion experiment.
4. Determine whether a historical PIT weather-forecast dataset is feasible.
5. Resolve `ESC-A` and `ESC-B` when implementation reaches those surfaces.
6. Keep the production reference-market recipe as TEST until empirical evidence exists.
