#!/usr/bin/env python3
"""
Phase 11: Hyperparameter tuning for score prediction models (post-pruning).

Tunes GradientBoostingRegressor separately for home and away score prediction.
Uses TimeSeriesSplit cross-validation on 2021-2023 training data.
Evaluates final performance on 2024 holdout set.

This script should be run after Phase 11 feature pruning to re-tune
hyperparameters for the reduced feature set (~21 features).

Usage:
    PYTHONPATH=. python scripts/tune_hyperparameters_v2.py
"""
...
def main():
    print("="*60)
    print("Phase 11: Hyperparameter Tuning (Post-Pruning)")
    print("="*60)
    ...
    # Parameter grid - same as Phase 6 for fair comparison
    param_grid = {
    ...
    # Get feature columns (now with Phase 11 pruning applied)
    feature_cols = _get_feature_columns(train_df)
    print(f"\nFeatures after Phase 11 pruning: {len(feature_cols)}")
    ...
    results = {
        "phase": 11,
        "description": "Post-pruning hyperparameter tuning",
        ...
        "phase6_baseline": {"spread_mae": 10.49, "total_mae": 10.07},
        "market": {"spread_mae": 9.63, "total_mae": 9.87},
    }

    output_path = Path(data_dir) / "tuning" / "phase11_results.json"
    ...
    print("\n" + "="*60)
    print("Recommended Parameters for ScoreModelV2 (Phase 11)")
    print("="*60)
