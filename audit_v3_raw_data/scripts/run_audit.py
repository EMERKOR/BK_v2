...
def audit_scores(schedule_index):
    ...
    for s in seasons:
        ...
        n_games, dup, bad_score, not_in_sched = 0, 0, 0, 0
        weeks = []
        ...
        per_season[s] = {
            ...
            "dup_game_id": dup, "bad_or_negative_scores": bad_score,
...
def audit_market(kind, subdir, value_cols, plausible, key_label):
    ...
    for s in seasons:
        ...
        n, dup, out_of_range, no_sched, null_vals = 0, 0, 0, 0, 0
        weeks = []
...
def audit_fp_coverage():
    for side, sub, ckey in [("defense", "I_fp_cov_def", "coverage_defense"),
        ...
        for f in files:
            ...
            headers_seen.setdefault(hkey, []).append(os.path.basename(f))
        ...
        inv(sub, f"data/RAW_fantasypoints/coverage/{side}/coverage_{side}_{{season}}_w{{week}}.csv",
            ...
            "one team per season/file-week", "season+week+team (post-glossary)",
            len(files), f"{seasons[0]}-{seasons[-1]}",
...
def audit_fp_wide():
    ...
    for fam, files in families.items():
        per_file = {}
        for f in files:
            m = re.search(r"_(\d{4})(?:_full)?\.csv$", os.path.basename(f))
...
def audit_fp_allowed():
    ...
    results["families"]["O_fp_allowed"] = {"per_position": per_pos}
    ...
    inv("O_fp_allowed", "data/RAW_fantasypoints/fp_allowed_{pos}_{season}_w{week}.csv",
        "fp_parsers.parse_fp_table (row1 header + Season filter)",
        "team-week-position", "season+week+team+position (post-glossary)",
